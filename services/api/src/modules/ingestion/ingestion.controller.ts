...
export class IngestionController {
  ...
  async ingestManual(@CurrentUser() user: AuthenticatedUser, @Body() dto: IngestManualDto, @Req() req: FastifyRequest) {
  ...
  /** §52.1 Capture "voice note" — see IngestionService.ingestVoiceNote's doc comment for what this does and doesn't do (yet). */
  @Throttle({ default: { limit: 20, ttl: 60_000 } })
  @Post("voice-note")
  ...
  @Get("voice-note/:sourceEventId/url")
  async voiceNoteUrl(@CurrentUser() user: AuthenticatedUser, @Param("sourceEventId") sourceEventId: string) {
    const url = await this.ingestion.voiceNoteUrl(sourceEventId, user.userId);
    if (!url) throw new NotFoundException({ code: "VOICE_NOTE_NOT_FOUND", message: "Voice note not found." });
    return { url };
  }

  /** §MSG-001 "Share-message extraction" — the share-sheet screenshot path (a text-message screenshot
   * shared via iOS/Android share sheet). See IngestionService.ingestShareScreenshot's own doc comment. */
  @Throttle({ default: { limit: 20, ttl: 60_000 } })
