...
export class SafeUrlFetcher {
  async fetchReadableText(rawUrl: string): Promise<{ title: string; text: string; finalUrl: string }> {
    const { body, finalUrl, contentType } = await this.fetchTrustedBytes(rawUrl);
  ...
  /**
   * The same SSRF-safe network layer as `fetchReadableText` above (hostname resolved and checked against
   * private/reserved IP ranges before every hop, manual redirect handling so a redirect destination is
   * re-validated rather than blindly followed, size-capped, timed out) but without the "must be readable
   * HTML" restriction — for callers that need the raw response body of a URL the user supplied in some
   * other content type (e.g. an ICS calendar feed, `text/calendar`), and therefore can't reuse
   * `fetchReadableText`'s content-type allowlist. Any caller that makes a server-side request to a
   * user-supplied URL MUST go through here (or `fetchReadableText`) rather than fetching directly — a
   * direct `fetch()`/library call bypasses every one of these checks and is a straightforward SSRF hole,
   * exactly the bug this method exists to prevent (found live: `IcsAdapter` used to call `node-ical`'s own
   * unguarded `fromURL`, which is a bare `fetch` with default redirect-following and no IP validation at
   * all — a scheduled, recurring SSRF primitive against internal infra/cloud metadata endpoints).
   */
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
      ...
      try {
        response = await fetch(current, {
          ...
          headers: { "user-agent": "VeynloBot/1.0 (+https://veynlo.app)", ...options.headers },
        });
      } catch {
      ...
      return { body, finalUrl: current.toString(), contentType: response.headers.get("content-type") ?? "" };
    }
