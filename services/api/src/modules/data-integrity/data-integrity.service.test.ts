  ...
  await db.insert(schema.purchases).values([
    {
      id: orphanPurchaseId,
      ownerUserId: ownerId,
      purchaseDate: { precision: "date", date: "2030-01-01", instantUtc: null, timezone: null, sourceText: null },
      confidenceBand: "verified",
      linkedEntityIds: [generateId("entity")],
    },
    {
      id: validPurchaseId,
      ownerUserId: ownerId,
...
describe("DataIntegrityService.scanForOrphans", () => {
  ...
  it("flags a purchase whose linkedEntityIds contains an id that resolves to nothing, and leaves a valid link alone", async () => {
    const result = await dataIntegrity.scanForOrphans();
    const purchaseFinding = result.jsonbArrayOrphans["purchases.linkedEntityIds"]!;
