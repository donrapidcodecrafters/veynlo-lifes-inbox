...
export class DataIntegrityService {
  ...
  /**
   * Check 3 — the six JSONB "*_entity_ids" arrays (purchases/bills/warranties/documents.linkedEntityIds,
   * tasks/calendarEvents.relatedEntityIds). Per their own schema comments (commerce.ts's
   * purchases.linkedEntityIds: "same generic link-to-any-resource pattern as documents.linkedEntityIds")
   * these aren't person-only despite PeopleService.linkedItems only ever reading them for that one case —
   * so an id is only flagged as orphaned if it resolves in NONE of the tables these arrays are ever
   * observed pointing at (people-links via CommerceService.setPersonLink/ScheduleService.setEventPersonLink,
   * plus the broader purchase/bill/warranty/document/calendar_event/task set documents' upload-time
   * `linkedResourceId` and the web app's history-section attach flow can point at).
   */
  private async scanJsonbArrayLinks(): Promise<Record<string, OrphanFinding>> {
    const sources: { key: string; rows: { id: string; linkedIds: string[] }[] }[] = await Promise.all([
      this.db
        .select({ id: schema.purchases.id, linkedIds: schema.purchases.linkedEntityIds })
        .from(schema.purchases)
        .then((rows) => ({ key: "purchases.linkedEntityIds", rows: rows.filter((r) => r.linkedIds.length > 0) })),
      this.db
        .select({ id: schema.bills.id, linkedIds: schema.bills.linkedEntityIds })
        .from(schema.bills)
        .then((rows) => ({ key: "bills.linkedEntityIds", rows: rows.filter((r) => r.linkedIds.length > 0) })),
      this.db
        .select({ id: schema.warranties.id, linkedIds: schema.warranties.linkedEntityIds })
        .from(schema.warranties)
        .then((rows) => ({ key: "warranties.linkedEntityIds", rows: rows.filter((r) => r.linkedIds.length > 0) })),
      this.db
