...
| Entity resolution | 🟡 Merchant-by-name with a real admin merge/unmerge UI + lineage (`apps/admin` `/dashboard/merchants`). No cross-source purchase/shipment/subscription reconciliation beyond what's in `ingestion.service.ts`. The owner-scoped `canonical_entities` knowledge-graph layer has a real first writer — `extractReceipt` creates one `canonical_entities` row per purchase line — and, as of 2026-08-31, a real second slice: `facts` gets a `purchase_details` row per asset entity (price/currency/quantity/purchase date, tagged with extraction method/version/confidence — exactly the shape the schema comment describes), and when `extractWarranty` finds a confident exact-match to that asset it now creates its own `canonical_entities` row (type `"warranty"`) plus a `relationships` row (`type: "covers"`) linking warranty → asset, with its own `warranty_expiration` fact. Verified live via the mocked-AI-boundary technique against real Postgres: a real receipt + warranty pair produced exactly 2 entities, 2 facts, and 1 relationship with the correct types/links. Found and fixed a real bug along the way: `purchase_lines.owner_asset_entity_id` had no `ON DELETE` behavior, which silently blocked full account deletion (a PRIV-002 regression) the instant a real user had both a purchase-derived asset entity and any FK cascade touching it — confirmed by reproducing the exact `FOREIGN KEY constraint` failure live, then fixed via migration `0013` (`ON DELETE SET NULL`) and re-verified the same test scenario's cleanup now succeeds. `entity_merge_lineage` (the owner-scoped equivalent of the merchant admin-merge UI) remains unwritten — there's now real data to merge over, but a user-facing merge/unmerge surface for personal knowledge-graph entities is a separate, larger UI feature not attempted this pass. |
...
| Browser extension | ✅ Built (`apps/browser-extension`, Manifest V3, Chromium-based browsers). Sign-in/out, save-page and save-selection via popup and right-click context menu, options page. **A prior "verified live via Playwright" claim on this row was wrong** — an earlier session's Playwright pass evidently never got far enough to hit a second authenticated request, because a real, session-wide bug made both save actions **completely broken for every real user after their very first sign-in**: the session cookie was being set unconditionally regardless of client platform, and Chrome's extension `host_permissions` grant made the extension's bearer-only background fetch silently pick up and resend that cookie anyway, which then tripped CSRF enforcement the extension (correctly) never satisfies. Found 2026-08-31 by a background agent that actually drove the real unpacked extension through multiple real authenticated actions in a real Chromium via `launchPersistentContext`, not just a single request — see `SECURITY.md`'s matching dated entry for the full root-cause and fix (the cookie is now platform-gated, and `AuthGuard` prefers bearer over cookie when both are present). Verified live post-fix: an extension sign-up now gets zero cookie, and its bearer token completes a real state-changing request with no CSRF header, with no regression to the web cookie/CSRF flow. Not yet: packaged/signed store build, real designed icons (placeholders are solid-color PNGs), Firefox (MV3 support differs). |
...
## 2026-08-31: full blueprint re-audit, interfaces, dead code, compliance docs

Full detail in `SECURITY.md`/`docs/DECISIONS.md` — this is a pointer, not a duplicate. Summary: added the
three §37 interfaces that didn't exist (`Cache`, `EmailProvider`/`PushProvider`, `BillingProvider`); fixed
a real mass-assignment gap, missing magic-byte upload validation, a missing PDF page-count cap, and moved
OCR to a background worker; added step-up auth + a session-management UI; added
`docs/SECURITY_CONTROLS.md`/`docs/THREAT_MODEL.md`/`docs/VENDOR_REGISTER.md` (blueprint §28.1/28.2/28.20,
none of which existed before). Ran `knip` across the whole workspace for a real dead-code sweep: removed 4
genuinely-unused npm dependencies, one dead `CardHeader` component, one dead `brandColor` export, and two
redundant re-export lines; added `expo-system-ui` as an explicit dependency (was implicitly required by
`app.json`'s `userInterfaceStyle: "automatic"` but never actually installed); verified every other
knip-flagged item is a false positive (browser-extension manifest-loaded scripts, Expo config-plugin entry
points, `pino-pretty`'s runtime-resolved transport, `@expo/config-plugins`'s use inside a plain `.js` config
plugin) rather than removing real, load-bearing code. Full workspace typecheck/lint/test clean throughout.

## 2026-08-31 (later the same day): real browser/simulator testing found 9 real bugs

Full detail in `SECURITY.md`'s matching dated entry — this is a pointer. Discovered mid-session that
Playwright (real Chromium) and a booted iOS Simulator are both actually usable in this environment, which
earlier passes incorrectly assumed wasn't true and fell back to static/curl-only verification — exactly
why several of these bugs survived multiple prior "verified" claims. Fixed, all re-verified live after the
fix: a CORS config gap that silently blocked every `PUT` request (meaning no notification-preference
change had ever actually saved through the web UI), a blanket "any 401 → redirect to sign-in" bug on both
web and mobile that made the accept-invite unauthenticated flow, the data-export step-up prompt, and
delete-account's wrong-password handling all unreachable through the real UI (backend logic for all three
was already correct — this was purely a client-side interception bug), a permanently-blank-page React
state bug on two auth pages, a discarded-error-message bug on the Connections page, an uncaught-rejection
crash on mobile's Connections screen, a cold-deep-link navigation crash on mobile, a real duplicate
attention-items seed-data bug, and a missing client-side role guard on the admin Admins page. Also ran a
real dead-code sweep (`knip` across the whole workspace): removed 4 genuinely-unused dependencies, a dead
component, a dead constant, and 2 redundant re-exports; added one genuinely-missing dependency
(`expo-system-ui`); verified everything else `knip` flagged is a false positive rather than real dead
code (browser-extension manifest-loaded scripts, Expo config plugins, runtime-resolved transports).

## 2026-09-01: voice note transcription built (closes the last §52.1 capture gap)

Voice notes previously recorded/scanned/stored/played back but were never transcribed — the mobile capture
form said outright "automatic transcription isn't available yet," and `source_events` rows for
`kind: "voice_note"` sat at `processingState: "filed"` forever with zero downstream processing, unlike
every other capture path (email/document/screenshot/manual text), which all run through AI classification/
extraction. The blocker had genuinely been real: Anthropic's Messages API has no audio-input content block,
so there was no call this app's one AI provider could make. Fixed with a deliberately different, fully
local model instead of a paid API: `WhisperVoiceTranscriptionService` (new `speech` module) runs a
quantized Whisper model (`Xenova/whisper-tiny.en`, ~40MB) entirely on-CPU via `@xenova/transformers` — no
API key, no per-call network request (only a one-time model-weight download from Hugging Face on first use,
cached under `~/.cache/veynlo/transformers` or `VOICE_TRANSCRIPTION_MODEL_CACHE_DIR` afterward; a
deployment with no outbound network access to huggingface.co needs that cache pre-seeded, e.g. baked into
the deploy image, before the first real voice note arrives). `ffmpeg-static` (a bundled, statically-linked
ffmpeg binary) decodes mobile's recorded m4a/AAC into the raw 16kHz mono PCM Whisper needs.

Wired into the exact same async-job pattern this codebase already uses for document OCR (§28.13): a new
`voice-transcription` BullMQ queue/worker (`queue-names.ts`/`queue-producer.service.ts`/`worker-main.ts`,
mirroring `documentOcr`'s shape) picks up the job the upload request enqueues, re-fetches the already-
stored audio blob, transcribes it, and — critically — feeds a successful transcript into the EXACT SAME
`IngestionService.classifyAndExtract` domain-classification/extraction pipeline manual-text capture already
uses, not a parallel or weaker path; a spoken "pick up the dry cleaning Friday" is classified/extracted
identically to a typed one. `source_events` gained a new encrypted `transcript` column (migration `0058`)
to hold it. A genuinely untranscribable clip (silence, corrupted audio, no speech detected) leaves the
source event exactly as a raw, playable recording — never fabricated, never crashes — matching this
codebase's "no half feature"/"never fabricate" discipline everywhere else. The mobile Inbox screen's capture
form copy was updated to match reality, and the Inbox list now shows the real transcript on a voice note's
card once ready (`InboxService.list` enrichment, mirroring its existing price-adjustment-deadline
enrichment shape) with a poll-while-pending effect (mirrors `app/data-export`'s identical 3-second poll)
so the card updates on its own once the background job finishes — no manual pull-to-refresh needed.

Real-Postgres integration test (`ingestion.voice-note.test.ts`) proves a captured voice note's transcript
reaches the same pipeline a typed manual-text capture would (asserting the actual `domain_classifier_v1` +
`calendar_event_extraction_v1` calls fire and a real `calendar_events` row gets created from a fake spoken
reminder), plus the untranscribable-clip path staying honest and crash-free — the transcription MODEL call
itself is stubbed via a new `FakeVoiceTranscriber`, mirroring this codebase's established `FakeModelProvider`
pattern for the Anthropic extraction model, while every downstream step is real.

## 2026-09-01: independent 6-agent MVP-completeness audit found 10 more real gaps, all fixed

Full detail in `SECURITY.md`'s matching dated entry — this is a pointer. Six parallel agents each
independently re-verified one §52.1 MVP requirement area live (told explicitly to distrust this file's own
prior claims), surfacing real gaps a single-pass review had missed. Highlights: a systemic dev-only DI bug
where `tsx watch` (the local worker process) silently drops any implicit-typed constructor dependency,
crashing the daily-brief job (fixed across 21 files with explicit `@Inject()`; production's real `tsc`
build was never affected); upload malware scanning was silently disabled despite a healthy ClamAV
container (one blank env var); duplicate bill/subscription ingestion was a real unfixed bug, now covered
by this repo's first-ever automated AI-pipeline test suite via a new reusable `FakeModelProvider`; **Sign
in with Apple**, built from scratch to the same honest "not configured" standard as Google/Microsoft;
**voice note capture (§52.1)**, the one MVP capture modality that was entirely unbuilt, now a complete
record/scan/store/playback pipeline (transcription was honestly deferred at the time — Claude's API has no
audio-input capability to fake — and has since been built for real via a local Whisper model; see this
file's own 2026-09-01 "voice note transcription built" entry above); admin visibility into BullMQ job
health and a cross-user privacy-requests worklist,
both previously nonexistent; a real seed-data gap that made Warranties impossible to verify locally;
quiet hours evaluated against the wrong timezone; a dead `historical_backfill_days` entitlement now
actually gates connector sync depth by plan. Deliberately not attempted, with reasoning recorded in
SECURITY.md: a dedicated Search UI (Ask already covers it) and real ranked retrieval for Ask (a genuine
quality gap, but one that needs a configured AI provider to do properly).
...
dashboard, automation/rule center. **Largely built, not "none started"** —
see `docs/PHASE2_PENDING_CREDENTIALS.md` for the itemized, code-complete
list and exactly what's still genuinely gated on a real credential/
partner account vs. an actual engineering gap: Plaid (`plaid.adapter.ts`),
Dropbox (`dropbox.adapter.ts`), Google Drive/Tasks and OneDrive/Microsoft
To Do connectors, the browser extension and packaged desktop apps (both
done, items 9-10 above), home/vehicle profiles (`assets.service.ts` —
`vehicleProfiles`/`propertyProfiles`), object-level sharing (`SharingService`,
generalized across documents/lists/purchases/properties/vehicles), RET-003/
004/006 returns-and-refunds work, and a real automation/rule center
(`AutomationService` — rule matching, `notify`/`add_task`/
`add_calendar_event` actions with approval + a 5-minute undo window, and an
account-wide kill switch; see this file's "Immediate next priorities"
section and `docs/PHASE2_PENDING_CREDENTIALS.md`'s "Explicitly deferred"
section for the action kinds still out of scope, e.g. no external/
connector-mutating action exists yet).

## Phase 3 — broaden life logistics

School/activity ingestion, trip mode, recall matching, location/context
reminders, smart-home signals, pet care, saved-memory resurfacing. **Built,
not "not started"** — every item in that list except smart-home has a real
backend and, for the user-facing ones, real web+mobile UI: LOC-003/004/005
location & context (`services/api/src/modules/location/`, on-device
geofencing, real today with zero external dependency; see
`docs/PHASE3_PENDING_CREDENTIALS.md` for the itemized list of exactly what's
still credential-gated within it, e.g. real traffic-aware travel time),
SCH-001/002/005/006/007 school/activity ingestion
(`services/api/src/modules/school/`), TRIP-001..009 trip mode
(`services/api/src/modules/trips/` — clustering, segment extraction, travel
documents, credits/vouchers, packing lists, disruption notifications; web
`/trips`, mobile `app/trips.tsx`), recall matching
(`services/api/src/modules/assets/recall-monitor.service.ts`, against
NHTSA/CPSC), the school-relevant slice of family transport conflicts, pet
care (`services/api/src/modules/pets/`; web `/life/pets`, mobile
`app/pet/[id].tsx`), and saved-memory resurfacing — a full "Saved Memories"
domain (`services/api/src/modules/memories/`, spec §29 SAVE-001..007: web
`/saved`, mobile `app/saved.tsx`) including a real `resurfacingRules`
mechanism (birthday/location-based rules, `ResurfacingService`), not
mentioned anywhere else in this file until this correction — verify
directly against the module before assuming it's covered by another row.
**Genuinely not started, by deliberate scope decision** (needs a partner
API/agreement, not just engineering time): smart-home (SMART-001/002/003 —
schema + adapter interface only, zero concrete provider implementations,
per spec's own "later expansion" scoping — see
...
- **Browser extension — free, mostly already possible.** Chrome's "Load
  ...
  instructions, not new code.
- ~~**Invite-gated sign-up — not started.**~~ — done 2026-09-01. A
  `signup_invites` table (`packages/db/src/schema/admin.ts`; sha256-hashed
  `codeHash` at rest, same scheme as `shareLinks.tokenHash` —
  `hashOpaqueToken` reused for both), a `SIGNUP_REQUIRES_INVITE` env flag
  (default off — zero behavior change unless a deployment opts in),
  `POST/GET /v1/admin/invites` + `POST /v1/admin/invites/:id/revoke`
  (`AdminGuard`-level, same tier as entitlement grant/revoke — not
  superadmin, which stays reserved for admin-account management), and an
  `apps/admin` "Invites" page (`/dashboard/invites`, cloned from the
  merchants page pattern — the plaintext code is shown exactly once at
  creation, same "shown once" UX as the admins page's temporary-password
  banner). `IdentityService.signUp()` validates and redeems a code inside a
  DB transaction holding a `pg_advisory_xact_lock` on the code's hash (same
  TOCTOU discipline as `household.service.ts`'s `invite()`), rejecting an
  invalid/expired/revoked/already-redeemed/wrong-email-bound code before
  any user row is written. A public `GET /v1/auth/config` exposes just the
  one boolean so the web (`sign-up/page.tsx`) and mobile (`sign-up.tsx`)
  sign-up screens can conditionally render the invite-code field. Real
  integration tests against real Postgres
  (`identity.signup-invite.test.ts`, `identity.signup-flag-off.test.ts`)
  cover invite creation, successful redemption + reuse blocked, expired
  rejected, revoked rejected, wrong-email-bound rejected, no-code-when-
  required rejected, and normal sign-up with no invite code when the flag
  is off. Verified live via Playwright against the real running apps with
  `SIGNUP_REQUIRES_INVITE` temporarily flipped on: created a real invite
  through the admin console, redeemed it through a real web sign-up,
  confirmed the admin table immediately showed it "redeemed", and confirmed
  a second sign-up attempt with the same code was rejected inline. Left
  `SIGNUP_REQUIRES_INVITE=false` in this dev `.env` afterward (its correct
  default) — flip it to `true` when this pre-launch testing-distribution
...
- **Fixed a real, session-wide bug**: none of the three frontends (web,
  ...
  around, rather than speculatively wiring error UI into every screen
  now.~~ — done 2026-09-01. **Pull-to-refresh audit**: a fresh grep of the
  whole `apps/mobile/app` tree for `setRefreshing` found three more
  handlers beyond the original five (`entities.tsx`, `automations.tsx`,
  `lists.tsx`) that already wrap their reload in `try/finally`, and
  confirmed all eight (the original five plus these three) now correctly
  stop the spinner on failure — verified live via Playwright by mocking a
  500 on `/v1/home` mid-refresh and confirming the indicator's `refreshing`
  prop returns to `false` rather than sticking. **Error/retry affordance**:
  built a shared `<FetchError>` component on web
  (`apps/web/src/components/ui/fetch-error.tsx`) and mobile
  (`apps/mobile/src/components/fetch-error.tsx`) — an inline "Something
  went wrong… Retry" block reusing SWR's `mutate()` (web) or a re-invoked
  `load()` (mobile's `useState`+`useEffect`/`useFocusEffect` screens) —
  and wired it in as the third branch (alongside loading/empty) on web's
  timeline/inbox/life/connections/documents/automations/ask/lists/settings-
  household pages and mobile's home/inbox/life/connections/timeline
  screens, distinct from the empty state and from the global 401 handling
  above (which is untouched — this is specifically for non-401 transient
  failures). The admin console's core user-lookup flow already handled
  fetch errors reasonably; its passive dashboard summary widgets
  (connector/model/queue health, privacy requests, audit log, feature
  flags) and the merchants/invites/admins list pages did not — added the
  same third-branch pattern there too (`SectionFetchError`, a local
  per-page component since admin has no shared UI kit). Verified live via
  Playwright route-mocking a 500 on each surface: web's `/inbox` and
  `/life` pages, the admin dashboard's connector-health section, and
  mobile's home tab all rendered the error block with a working Retry that
  recovered the real data once the mock was removed, with no console
  errors.
...
- ~~No automated backup/restore drills~~ — done 2026-08-31. `pnpm db:backup`
  (`packages/db/src/scripts/backup.ts`) runs a real `pg_dump` against
  `DATABASE_URL`; `pnpm db:restore-drill` (`restore-drill.ts`) does the full
  cycle in one command — dump, restore into a throwaway isolated database on
  the same server, verify row counts match exactly across 16 tables spanning
  identity/commerce/documents/the knowledge graph, then always clean up
  (drops the drill database and deletes the dump, even on failure). Wired
  into CI (`.github/workflows/ci.yml`) so it runs on every build against the
  freshly migrated-and-seeded database, not only on the blueprint's quarterly
  minimum. Found and fixed a real bug on the first live run: the drill
  correctly detected all 16 tables matching, but cleanup then failed —
  `dropdb` refused because an open connection pool from the row-count check
  was still attached to the drill database; fixed by using a plain
  `pg.Client` that's explicitly `.end()`-ed per query rather than a pooled
  drizzle client with no natural "done" signal for a short-lived script.
  Verified live end-to-end twice in a row with a clean exit both times (no
  leftover drill database, no leftover dump file). Production backup
  *policy* (retention, cross-region copy, Vault Lock) is still an
  infrastructure decision for whenever a real AWS account exists — see
  docs/DECISIONS.md.
- Structured audit logging exists and is used by the admin console
  ...
  correctly), and now Inbox corrections too (done 2026-08-31 —
  `InboxService.correct()` writes an `inbox.correct` audit event with the
  correction payload, verified live end-to-end against real seed data:
  a real correction changed the domain row and produced exactly one new
  audit row with the right actor/resource/payload). Sharing changes still
  don't write to it — no sharing feature exists yet (Phase 2).
...
- ~~Session refresh-token rotation is not implemented~~ — done 2026-08-31.
  Native (bearer-token) clients now get a rotating refresh token alongside
  the access-token session (`sessions.refreshTokenHash`/`previousRefreshTokenHash`/
  `refreshExpiresAt`, `POST /v1/auth/refresh`), with reuse detection: a
  replayed, already-rotated-away token revokes the whole session rather than
  being silently re-honored. Web's cookie flow is unchanged — it never had a
  client-side refresh token to rotate. Verified live end-to-end via curl:
  two successful rotations in a row, then a replay of the first-generation
  token correctly rejected, then a replay of the second-generation (now
  superseded) token correctly detected as reuse and revoked the session,
  confirmed via a real `sessions.revoked_at` check in Postgres. Mobile's
  `token-store`/`api-client`/`auth-context` now store and use the refresh
  token (typechecked; not independently re-verified live via `expo start
  --web` this pass, unlike the backend half). See docs/DECISIONS.md for the
  full session's changes.
