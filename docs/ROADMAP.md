...
| Ingestion pipeline | ✅ Deterministic prefilter + AI domain classification/extraction for receipts, bills, calendar events, and warranties (`extractWarranty`, `warranties` table, `GET /v1/warranties`, surfaced on Life and Timeline). Travel is partially covered by the calendar extractor; school/home/vehicle extractors not started. **Found while building domain detail views**: nothing anywhere in the ingestion pipeline ever writes to `subscriptions`/`recurring_streams` — there is no `extractSubscription` method, unlike every other §52.1 MVP domain. The whole "Subscriptions" section on the Life page has been seed-data-only this entire time, the same class of silently-empty-for-every-real-user gap as `attention_items` was before it was fixed earlier this session. Not yet fixed — a real subscription/recurring-charge detector is a genuine remaining MVP gap, tracked as a follow-up. |
...
| Billing/entitlements | ✅ Stripe checkout + webhook + entitlement resolution, plus a real RevenueCat webhook handler (`revenuecat.service.ts`) normalizing App Store/Play Store/web entitlements into the same table — live-tested including a real bug found and fixed via a synthetic webhook call. Not built: the mobile IAP SDK/paywall UI itself (deliberately deferred — see `SECURITY.md`/roadmap testing-distribution section, since it can't be meaningfully tested without the paid Apple/Google developer accounts either way). **Also not built, found while auditing for the search/connection-deletion bugs above**: `packages/core/src/entitlements/plans.ts` defines 15 real `CapabilityKey`s (connector count, Ask queries/day, document storage, automation rules, household size, etc.) with an explicit comment that "every gate in the product should check a capability key here" — `resolveCapability` correctly computes these in `billing.service.ts`, but nothing outside that one file ever calls it. No connector-count check, no Ask throttle, no storage cap beyond a flat 25MB-per-file limit, no household-size check — a free-tier account today has no actual usage limit on anything. Deliberately not fixed in this pass: there's no billing/upgrade UI anywhere in `apps/web`/`apps/mobile` yet (no page references `entitlements`/`checkout`/`planKey`), so unlike the search/connection-deletion bugs, nothing currently makes a false promise to a user about this — adding hard limits with no corresponding "you've hit your plan limit, upgrade here" UI would just be a confusing dead end. Real quota enforcement belongs with real billing UI, which is Phase 2 scope. |
