...
| 10 | Never commit production secrets or personal test data | Verified clean | `.gitignore` covers `.env`/`.env.local`/`.env.*.local` (excluding `.env.example`). `git log --all --oneline -- '*.env' '**/.env'` returns nothing — no `.env` file has ever been committed. No secret-scanning findings (CI's `gitleaks` job is blocking, 0 findings as of the last SECURITY.md pass). |
