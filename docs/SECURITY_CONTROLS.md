...
| V2.2 | Anti-automation on auth endpoints (rate limiting) | Applicable | Two layers: per-IP `@Throttle` on sign-in/sign-up/forgot-password/reset-password, plus a per-ACCOUNT failure counter on sign-in (10 per 15 min, Redis-backed via `Cache`). The per-IP layer alone does not address credential stuffing, which is distributed by definition — and it uses ThrottlerModule's in-memory storage, so its counters are per-process and reset on deploy. | No | Yes (live 429 confirmed; per-account layer covered by `identity.signin-throttle.test.ts`) | `identity.controller.ts`, `identity.service.ts` |
