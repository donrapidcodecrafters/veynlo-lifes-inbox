...
export default function ShareExtension(props: InitialProps) {
  // A shared image/file (from Photos, Files, another app's "Share" sheet) takes priority over any
  // accompanying caption text — sharing a receipt photo with a one-line caption should capture the photo,
  // not discard it in favor of the caption. Only the first file is supported, same one-file-per-capture
  // shape as the in-app document upload flow.
  const sharedFilePath = props.images?.[0] ?? props.files?.[0];
  const [subject, setSubject] = useState(sharedFilePath ? "Shared file" : props.url ? "Shared link" : "Shared text");
  ...
  function save() {
    setSaving(true);
    // `sharedFilePath` is a path inside the App Group container (com.apple.security.application-groups —
    // see VeynloShareExtension.entitlements) both this extension and the main app can read, so only the
    // short path itself needs to travel through the deep link — never raw file bytes, which the naive
    // "&"/"="-split URL transport this hands off through (see this file's own top comment) couldn't carry
    // at any real file size anyway.
    const path = sharedFilePath
      ? `capture?subject=${encodeURIComponent(subject)}&filePath=${encodeURIComponent(sharedFilePath)}`
      : `capture?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    openHostApp(path);
  }
